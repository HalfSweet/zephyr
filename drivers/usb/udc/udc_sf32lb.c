/*
 * Copyright (c) 2024 SiFli Technologies(Nanjing) Co., Ltd.
...
static void sf32lb_flush_ctrl_fifo(uintptr_t base)
{
	uint8_t csr0 = HWREGB(base + USB_TXCSRL_OFFSETX(0));

	if ((csr0 & (USB_CSRL0_RXRDY | USB_CSRL0_TXRDY)) == 0U) {
		return;
	}

	HWREGB(base + USB_TXCSRH_OFFSETX(0)) = USB_CSRH0_FLUSH;
	if (HWREGB(base + USB_TXCSRL_OFFSETX(0)) & (USB_CSRL0_RXRDY | USB_CSRL0_TXRDY)) {
		HWREGB(base + USB_TXCSRH_OFFSETX(0)) = USB_CSRH0_FLUSH;
	}
}

static void sf32lb_flush_in_fifo(uintptr_t base, uint8_t ep_idx)
{
	uint8_t csr = HWREGB(base + USB_TXCSRL_OFFSETX(ep_idx));
	...
	HWREGB(base + USB_TXCSRL_OFFSETX(ep_idx)) = flush;
	if (HWREGB(base + USB_TXCSRL_OFFSETX(ep_idx)) &
	    (USB_TXCSRL1_TXRDY | USB_TXCSRL1_FIFONE)) {
		HWREGB(base + USB_TXCSRL_OFFSETX(ep_idx)) = flush;
	}
}

static void sf32lb_flush_out_fifo(uintptr_t base, uint8_t ep_idx)
{
	uint8_t csr = HWREGB(base + USB_RXCSRL_OFFSETX(ep_idx));
	...
	HWREGB(base + USB_RXCSRL_OFFSETX(ep_idx)) = flush;
	if (HWREGB(base + USB_RXCSRL_OFFSETX(ep_idx)) & USB_RXCSRL1_RXRDY) {
		HWREGB(base + USB_RXCSRL_OFFSETX(ep_idx)) = flush;
	}
}
...
{
	uintptr_t base = ((const struct udc_sf32lb_config *)dev->config)->base;

	if (ep_idx == 0U) {
		sf32lb_flush_ctrl_fifo(base);
		return;
	}

	if (in) {
		sf32lb_flush_in_fifo(base, ep_idx);
	} else {
		sf32lb_flush_out_fifo(base, ep_idx);
	}
}
...
static void sf32lb_reset_setup_queue(const struct device *dev)
{
	...
	priv->setup_epoch++;
	priv->setup_worker_running = false;
...
static int sf32lb_queue_setup(const struct device *dev,
			      const struct usb_setup_packet *setup)
{
	...
	key = k_spin_lock(&priv->setup_lock);
	if (!priv->setup_worker_running) {
		priv->setup_worker_running = true;
		submit = true;
	}
	k_spin_unlock(&priv->setup_lock, key);
...
static struct udc_ep_config *sf32lb_ep_cfg_from_idx(const struct device *dev,
						    uint8_t ep_idx, bool in)
{
	return udc_get_ep_cfg(dev, in ? (USB_EP_DIR_IN | ep_idx) : ep_idx);
}

static uint8_t sf32lb_ep_phys_idx(const struct device *dev, uint8_t ep)
{
	ARG_UNUSED(dev);
...
{
	return ((priv->setup.bmRequestType & USB_EP_DIR_IN) == 0U) &&
	       (priv->setup.wLength != 0U);
}
...
static void sf32lb_ep0_commit_status_in(const struct device *dev)
{
	...
	HWREGB(cfg->base + USB_TXCSRL_OFFSETX(0)) =
		USB_CSRL0_RXRDYC | USB_CSRL0_DATAEND;
...
static void sf32lb_fail_out_overflow(const struct device *dev,
				     ...
				     uint8_t ep_idx, uint16_t count)
{
	...
	LOG_ERR("ep 0x%02x overflow, packet %u > remaining %u",
		ep_cfg->addr, count, xfer->remaining);

	sf32lb_flush_out_fifo(cfg->base, ep_idx);
...
static void sf32lb_abort_ep_xfer(const struct device *dev,
				 struct udc_ep_config *ep_cfg)
{
	struct udc_sf32lb_data *priv = udc_get_private(dev);
	const struct udc_sf32lb_config *cfg = dev->config;
	uint8_t ep_idx = sf32lb_ep_phys_idx(dev, ep_cfg->addr);
...
static int sf32lb_tx_start(const struct device *dev,
			   struct udc_ep_config *ep_cfg)
{
	const struct udc_sf32lb_config *cfg = dev->config;
	struct udc_sf32lb_data *priv = udc_get_private(dev);
...
static int sf32lb_rx_arm(const struct device *dev,
			 struct udc_ep_config *ep_cfg)
{
	const struct udc_sf32lb_config *cfg = dev->config;
	struct udc_sf32lb_data *priv = udc_get_private(dev);
...
static void sf32lb_handle_setup_work(struct k_work *work)
{
	...
	for (;;) {
		...
		key = k_spin_lock(&priv->setup_lock);
		if (k_msgq_num_used_get(&priv->setup_msgq) == 0U) {
			priv->setup_worker_running = false;
			k_spin_unlock(&priv->setup_lock, key);
			break;
		}
...
static void sf32lb_complete_in(const struct device *dev, uint8_t ep_idx)
{
	...
	if (HWREGB(((const struct udc_sf32lb_config *)dev->config)->base +
		   USB_TXCSRL_OFFSETX(ep_idx)) & USB_TXCSRL1_UNDRN) {
		HWREGB(((const struct udc_sf32lb_config *)dev->config)->base +
		       USB_TXCSRL_OFFSETX(ep_idx)) &= ~USB_TXCSRL1_UNDRN;
	}
	...
	sf32lb_xfer_reset(xfer);
	udc_ep_set_busy(ep_cfg, false);
	HWREGH(((const struct udc_sf32lb_config *)dev->config)->base +
	       MUSB_TXIE_OFFSET) &= ~BIT(ep_idx);
...
static int sf32lb_udc_ep_enqueue(const struct device *dev,
...
static int sf32lb_udc_ep_dequeue(const struct device *dev,
				 struct udc_ep_config *const cfg_ep)
{
...
static int sf32lb_udc_ep_set_halt(const struct device *dev,
				  struct udc_ep_config *const ep_cfg)
{
	const struct udc_sf32lb_config *cfg = dev->config;
	uint8_t ep_idx = sf32lb_ep_phys_idx(dev, ep_cfg->addr);
...
static int sf32lb_udc_ep_clear_halt(const struct device *dev,
				    struct udc_ep_config *const ep_cfg)
{
	const struct udc_sf32lb_config *cfg = dev->config;
	uint8_t ep_idx = sf32lb_ep_phys_idx(dev, ep_cfg->addr);
...
static int sf32lb_udc_ep_try_config(const struct device *dev,
				    struct udc_ep_config *const ep_cfg)
{
	ARG_UNUSED(dev);
	ARG_UNUSED(ep_cfg);
...
static int sf32lb_udc_ep_enable(const struct device *dev,
				struct udc_ep_config *const ep_cfg)
{
	const struct udc_sf32lb_config *cfg = dev->config;
	uint8_t ep_idx = sf32lb_ep_phys_idx(dev, ep_cfg->addr);
	...
	if (USB_EP_DIR_IS_IN(ep_cfg->addr)) {
		...
		HWREGB(cfg->base + USB_TXCSRH_OFFSETX(ep_idx)) =
			USB_TXCSRH1_MODE |
			((ep_type == USB_EP_TYPE_ISO) ? USB_TXCSRH1_ISO : 0U);
	} else {
		if (HWREGB(cfg->base + USB_RXCSRL_OFFSETX(ep_idx)) & USB_RXCSRL1_RXRDY) {
			HWREGB(cfg->base + USB_RXCSRL_OFFSETX(ep_idx)) =
...
static int sf32lb_udc_ep_disable(const struct device *dev,
				 struct udc_ep_config *const ep_cfg)
{
	const struct udc_sf32lb_config *cfg = dev->config;
	uint8_t ep_idx = sf32lb_ep_phys_idx(dev, ep_cfg->addr);
...
static int sf32lb_udc_test_mode(const struct device *dev, const uint8_t mode,
				const bool dryrun)
{
	ARG_UNUSED(dev);
	ARG_UNUSED(mode);
...
static const struct udc_api sf32lb_udc_api = {
	...
	.ep_try_config = sf32lb_udc_ep_try_config,
	...
	.test_mode = sf32lb_udc_test_mode,
};
...
static int udc_sf32lb_driver_init(const struct device *dev)
{
	...
	k_msgq_init(&priv->setup_msgq, priv->setup_msgq_buffer,
		    sizeof(struct sf32lb_setup_msg), SF32LB_SETUP_QUEUE_DEPTH);
	priv->setup_epoch = 1U;
...
#define UDC_SF32LB_INIT(inst)                                                            \
	...
	{                                                                                \
		IRQ_CONNECT(DT_INST_IRQN(inst), DT_INST_IRQ(inst, priority),            \
			    sf32lb_udc_isr, DEVICE_DT_INST_GET(inst), 0);               \
	...
	DEVICE_DT_INST_DEFINE(inst, udc_sf32lb_driver_init, NULL,                        \
			      &udc_data_##inst, &udc_sf32lb_config_##inst,             \
			      POST_KERNEL, CONFIG_KERNEL_INIT_PRIORITY_DEVICE,        \
			      &sf32lb_udc_api);

DT_INST_FOREACH_STATUS_OKAY(UDC_SF32LB_INIT)
