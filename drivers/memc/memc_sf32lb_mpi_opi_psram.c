...
static void mpi_wait_complete(uintptr_t mpi)
{
...
static int mpi_calibrate_delay(const struct device *dev)
{
	...
	LOG_DBG("Calibration: delay=%u, sck=%u, dqs=%u",
		delay, data->sck_delay, data->dqs_delay);
...
static void mpi_manual_cmd(uintptr_t mpi, bool is_write, uint8_t dmode, uint8_t dcyc,
			   uint8_t abmode, uint8_t absize, uint8_t adsize,
			   uint8_t admode, uint8_t imode)
{
	uint32_t ccr1 = 0U;
...
static void mpi_psram_reset(const struct device *dev)
{
	const struct memc_sf32lb_mpi_opi_psram_config *cfg = dev->config;
	uintptr_t mpi = cfg->mpi_base;

	/* Configure reset command: write mode, no data, 1-byte AB, 32-bit addr, OPI mode */
	mpi_manual_cmd(mpi, true, CCR_MODE_NONE, 0, CCR_MODE_OCT, 0,
		       CCR_ADSIZE_32, CCR_MODE_OCT, CCR_MODE_OCT);

	/* Send reset command */
	sys_write32(0U, mpi + MPI_AR1);
	sys_write32(OPSRAM_CMD_RESET, mpi + MPI_CMDR1);
	mpi_wait_complete(mpi);

	/* Wait for PSRAM to reset */
	mpi_delay_us(3);
}

static void mpi_mr_write(const struct device *dev, uint8_t addr, uint8_t value)
{
	const struct memc_sf32lb_mpi_opi_psram_config *cfg = dev->config;
	uintptr_t mpi = cfg->mpi_base;

	/* Configure MR write command: write mode, OPI data, no dummy, 32-bit addr, OPI mode */
	mpi_manual_cmd(mpi, true, CCR_MODE_OCT, 0, CCR_MODE_NONE, 0,
		       CCR_ADSIZE_32, CCR_MODE_OCT, CCR_MODE_OCT);
...
static __maybe_unused uint8_t mpi_mr_read(const struct device *dev, uint8_t addr)
{
	...
	mpi_manual_cmd(mpi, false, CCR_MODE_OCT, rdcyc - 1, CCR_MODE_NONE, 0,
		       CCR_ADSIZE_32, CCR_MODE_OCT, CCR_MODE_OCT);
...
static void mpi_set_fixlat(const struct device *dev, bool fix, uint8_t r_lat, uint8_t w_lat)
{
	...
	/* Update APM32CR read/write cycles (TCPHR=read, TCPHW=write) */
	// uint32_t apm32cr = sys_read32(mpi + MPI_APM32CR);
	// apm32cr &= ~(MPI_APM32CR_TCPHR_Msk | MPI_APM32CR_TCPHW_Msk);
	// apm32cr |= FIELD_PREP(MPI_APM32CR_TCPHR_Msk, r_lat - 1);
	// apm32cr |= FIELD_PREP(MPI_APM32CR_TCPHW_Msk, w_lat - 1);
	// sys_write32(apm32cr, mpi + MPI_APM32CR); 
}
...
static void mpi_set_cs_timing(const struct device *dev, uint32_t freq)
{
	...
	dcr = sys_read32(mpi + MPI_DCR);
	dcr &= ~(MPI_DCR_CSLMAX_Msk | MPI_DCR_CSLMIN_Msk | MPI_DCR_CSHMIN_Msk | MPI_DCR_TRCMIN_Msk);
...
static int memc_sf32lb_mpi_opi_psram_init(const struct device *dev)
{
	...
	// dcr = sys_read32(0x500ca05c);

	// dcr |= 1;
	// sys_write32(dcr, 0x500ca05c); 
	// mpi_delay_us(10000);
	...
	LOG_INF("PSRAM initialized: base=0x%08lx, size=%u bytes",
		(unsigned long)cfg->psram_base, cfg->size);
...
#define MEMC_SF32LB_MPI_OPI_PSRAM_INIT(n)                                                           \
	...
	static struct memc_sf32lb_mpi_opi_psram_data memc_sf32lb_mpi_opi_psram_data_##n;            \
                                                                                                   \
	static const struct memc_sf32lb_mpi_opi_psram_config memc_sf32lb_mpi_opi_psram_config_##n = \
		{                                                                                  \
